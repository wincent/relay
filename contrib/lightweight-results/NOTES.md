...
- Make a transform that would enable component-authors to continue to read data
  ...
  experimented with lazily-contructed reader methods and noted some speed ups,
...
- **reverse**: Time, in milliseconds, to run the data through the inverse
  equivalent to the pipeline. In other words, this is the "decoding" counterpart
  to the "encoding" step. I did not implement this inverse operation for all of
  the pipelines.
...
procssening and the final gzipping step.

Even the smallest first phase ("arrayrefize") format, which is 71.30% of the
size of the basic object+stringified format, ends up compressing worse than the
naive "redundant" JSON representation (8.67% compared to 6.91%, even with the
small boost from including MessagePack in the pipeline).

In short, if you try to eliminate redundancy by applying some kind of clever
transformation of restructuring you data object before piping it to gzip, you
...
artificial one at that, so I wanted do test with another data point.
...
the query to get bigger. Although gzip does end up undoing a lot of the damage,
the simple "passthrough"+gzip still comes out in front. Notably, and unlike the
TodoMVC example, addind an intermediate MessagePack step did not improve the
overall results.
